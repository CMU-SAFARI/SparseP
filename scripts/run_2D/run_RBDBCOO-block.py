...
def run(input_path):
    ...
    NR_TASKLETS = [16]
    NR_DPUS = [2048, 2528]
    NR_VERT_PARTITIONS = [2, 4, 8, 16, 32]
    DATATYPES = ["int8", "int32", "fp32", "fp64"]
    ...
    pwd = os.getcwd()
    path = pwd.replace("scripts/run_2D", "") + "spmv/2D/RBDBCOO-block"
    os.chdir(path)
            ...
            for r in NR_DPUS:
                for t in NR_TASKLETS:
                    for c in NR_VERT_PARTITIONS:
                        # Balance NNZs across tasklets - Coarse-grained - Fine-grained data transfers (default)
