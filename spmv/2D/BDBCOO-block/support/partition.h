 ...
 */
struct partition_info_t *partition_init(struct BDBCOOMatrix *bdbcooMtx, uint32_t nr_of_dpus, uint32_t max_ranks, uint32_t nr_of_tasklets) {
    struct partition_info_t *part_info;
    part_info = (struct partition_info_t *) malloc(sizeof(struct partition_info_t));

    part_info->brow_split = (uint32_t *) malloc(2 * bdbcooMtx->vert_partitions * (bdbcooMtx->horz_partitions + 2) * sizeof(uint32_t));
    part_info->blocks_dpu = (uint32_t *) calloc(bdbcooMtx->vert_partitions * (bdbcooMtx->horz_partitions + 1), sizeof(uint32_t));
    part_info->nnzs_dpu = (uint32_t *) calloc(bdbcooMtx->vert_partitions * (bdbcooMtx->horz_partitions + 1), sizeof(uint32_t));
 ...
 */
void partition_by_block(struct BDBCOOMatrix *bdbcooMtx, struct partition_info_t *part_info) {

    uint32_t c, i, j;
    uint32_t total_blocks = 0;
    uint32_t total_block_rows = 0;
    for(c = 0; c < bdbcooMtx->vert_partitions; c++) {
        // Compute the matrix splits.
        uint32_t sptr_offset = c * (bdbcooMtx->horz_partitions + 1);
        uint32_t dptr_offset = c * bdbcooMtx->horz_partitions;
        uint32_t rptr_offset = 2 * c * bdbcooMtx->horz_partitions;
        uint32_t block_cnt = bdbcooMtx->blocks_per_vert_partition[c];
        uint32_t block_per_split = block_cnt / bdbcooMtx->horz_partitions;
        ...
        for (i = 0; i < bdbcooMtx->blocks_per_vert_partition[c]; i++) {
            curr_block++;
            curr_nnz += bdbcooMtx->nnz_per_block[total_blocks+i];
            if (curr_block >= block_per_split) {
                row_start = bdbcooMtx->bind[total_blocks + i].rowind + 1;
                ++split_cnt;
                if (split_cnt <= bdbcooMtx->horz_partitions) {
                    part_info->brow_split[rptr_offset + 2 * (split_cnt - 1) + 1] = row_start;
                    if (i == (bdbcooMtx->blocks_per_vert_partition[c] - 1)) { 
                        part_info->brow_split[rptr_offset + 2 * split_cnt] = row_start;
                    } else if (bdbcooMtx->bind[total_blocks + i].rowind == bdbcooMtx->bind[total_blocks + i + 1].rowind) {
        ...
        if (curr_block < block_per_split && split_cnt <= bdbcooMtx->horz_partitions) {
            split_cnt++;
            part_info->brow_split[rptr_offset + 2 * (split_cnt - 1) + 1] = bdbcooMtx->num_block_rows;
            part_info->brow_split[rptr_offset + 2 * split_cnt] = bdbcooMtx->num_block_rows;
            part_info->blocks_dpu[dptr_offset + split_cnt - 1] = curr_block;
            part_info->nnzs_dpu[dptr_offset + split_cnt - 1] = curr_nnz;
        }
        ...
        if (split_cnt > bdbcooMtx->horz_partitions) {
            part_info->brow_split[rptr_offset + 2 * (bdbcooMtx->horz_partitions - 1) + 1] = bdbcooMtx->num_block_rows;
            part_info->brow_split[rptr_offset + 2 * bdbcooMtx->horz_partitions] = bdbcooMtx->num_block_rows;
            part_info->blocks_dpu[dptr_offset + bdbcooMtx->horz_partitions - 1] += curr_block;
            part_info->nnzs_dpu[dptr_offset + bdbcooMtx->horz_partitions - 1] += curr_nnz;
        }

        // If there are remaining threads create empty partitions
        for (i = split_cnt + 1; i <= bdbcooMtx->horz_partitions; i++) {
            part_info->brow_split[rptr_offset + 2 * (i - 1) + 1] = bdbcooMtx->num_block_rows;
            part_info->brow_split[rptr_offset + 2 * i] = bdbcooMtx->num_block_rows;
            part_info->blocks_dpu[dptr_offset + i - 1] = 0;
            part_info->nnzs_dpu[dptr_offset + i - 1] = 0;
        }

        total_blocks += bdbcooMtx->blocks_per_vert_partition[c];
        total_block_rows += bdbcooMtx->num_block_rows;
    }
    ...
    for (c = 0; c < bdbcooMtx->vert_partitions; c++) {
        for (i = 0; i < bdbcooMtx->horz_partitions; i++) {
            total_blocks += part_info->blocks_dpu[c * bdbcooMtx->horz_partitions + i];
            total_nnzs += part_info->nnzs_dpu[c * bdbcooMtx->horz_partitions + i];
        }
    }
    assert(total_blocks == bdbcooMtx->nblocks && "Invalid partitioning!");
    assert(total_nnzs == bdbcooMtx->nnz && "Invalid partitioning!");
 ...
 */ 
void partition_tsklt_by_block(struct BDBCOOMatrix *bdbcooMtx, struct partition_info_t *part_info, int dpu, int nr_of_tasklets, int nr_of_dpus, int acc_blocks) {
    ...
    part_info->block_split_tasklet[block_offset] = 0;
 ...
 */ 
void partition_tsklt_by_nnz(struct BDBCOOMatrix *bdbcooMtx, struct partition_info_t *part_info, int dpu, int nr_of_tasklets, int nr_of_dpus, int acc_blocks) {
    ...
    for (i = 0; i < part_info->blocks_dpu[dpu]; i++) {
        curr_nnz += bdbcooMtx->nnz_per_block[acc_blocks + i];
