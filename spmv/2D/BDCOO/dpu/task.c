...
int main() {
    ...
    if(tasklet_id == 0) { 
        ...
        uint32_t acc_init = 0;
#if INT8
        iter = (max_rows_per_dpu >> 11);
#elif INT16
        iter = (max_rows_per_dpu >> 10);
#elif INT32
        iter = (max_rows_per_dpu >> 9);
#elif INT64
        iter = (max_rows_per_dpu >> 8);
#elif FP32
        iter = (max_rows_per_dpu >> 9);
#elif FP64
        iter = (max_rows_per_dpu >> 8);
...
#endif
        for(i=0; i < iter; i++) {
            mram_write(temp, (__mram_ptr void *) (mram_base_addr_y), 2048);
            mram_base_addr_y += 2048;
            acc_init += 256;
        }
        ...
        for(i=acc_init; i < iter; i++) {
            mram_write(temp, (__mram_ptr void *) (mram_base_addr_y), 8);
            mram_base_addr_y += 8;
        }
        mem_reset(); // Reset the heap
