 ...
 */
struct partition_info_t *partition_init(struct BDBCSRMatrix *bdbcsrMtx, uint32_t nr_of_dpus, uint32_t max_ranks, uint32_t nr_of_tasklets) {
    struct partition_info_t *part_info;
    part_info = (struct partition_info_t *) malloc(sizeof(struct partition_info_t));

    part_info->brow_split = (uint32_t *) malloc(bdbcsrMtx->vert_partitions * (bdbcsrMtx->horz_partitions + 2) * sizeof(uint32_t));
    part_info->blocks_dpu = (uint32_t *) calloc(bdbcsrMtx->vert_partitions * (bdbcsrMtx->horz_partitions + 1), sizeof(uint32_t));
    part_info->nnzs_dpu = (uint32_t *) calloc(bdbcsrMtx->vert_partitions * (bdbcsrMtx->horz_partitions + 1), sizeof(uint32_t));
...
/** 
 * @brief load-balance blocks at block-row granularity across DPUs
 */
void partition_by_block(struct BDBCSRMatrix *bdbcsrMtx, struct partition_info_t *part_info) {

    uint32_t c, i, j;
    uint32_t total_blocks = 0;
    for(c = 0; c < bdbcsrMtx->vert_partitions; c++) {
        // Compute the matrix splits.
        uint32_t sptr_offset = c * (bdbcsrMtx->horz_partitions + 1);
        uint32_t dptr_offset = c * bdbcsrMtx->horz_partitions;
        uint32_t rptr_offset = c * (bdbcsrMtx->num_block_rows + 1);
        uint32_t block_cnt = bdbcsrMtx->blocks_per_vert_partition[c];
        uint32_t block_per_split = block_cnt / bdbcsrMtx->horz_partitions;
        ...
        for (i = 0; i < bdbcsrMtx->num_block_rows; i++) {
            curr_block += bdbcsrMtx->browptr[rptr_offset + i+1] - bdbcsrMtx->browptr[rptr_offset + i];
            for (j = bdbcsrMtx->browptr[rptr_offset + i]; j < bdbcsrMtx->browptr[rptr_offset + i+1]; j++)
                curr_nnz += bdbcsrMtx->nnz_per_block[total_blocks+j];
            if (curr_block >= block_per_split) {
                row_start = i + 1;
                ++split_cnt;
                if (split_cnt <= bdbcsrMtx->horz_partitions) {
        ...
        if (curr_block < block_per_split && split_cnt <= bdbcsrMtx->horz_partitions) {
            split_cnt++;
            part_info->brow_split[sptr_offset + split_cnt] = bdbcsrMtx->num_block_rows;
            part_info->blocks_dpu[dptr_offset + split_cnt - 1] = curr_block;
            part_info->nnzs_dpu[dptr_offset + split_cnt - 1] = curr_nnz;
        }
        ...
        if (split_cnt > bdbcsrMtx->horz_partitions) {
            part_info->brow_split[sptr_offset + bdbcsrMtx->horz_partitions] = bdbcsrMtx->num_block_rows;
            part_info->blocks_dpu[dptr_offset + bdbcsrMtx->horz_partitions - 1] += curr_block;
            part_info->nnzs_dpu[dptr_offset + bdbcsrMtx->horz_partitions - 1] += curr_nnz;
        }

        // If there are remaining threads create empty partitions
        for (i = split_cnt + 1; i <= bdbcsrMtx->horz_partitions; i++) {
            part_info->brow_split[sptr_offset + i] = bdbcsrMtx->num_block_rows;
            part_info->blocks_dpu[dptr_offset + i - 1] = 0;
            part_info->nnzs_dpu[dptr_offset + i - 1] = 0;
        }

        total_blocks += bdbcsrMtx->blocks_per_vert_partition[c];
    }
    ...
    for (c = 0; c < bdbcsrMtx->vert_partitions; c++) {
        for (i = 0; i < bdbcsrMtx->horz_partitions; i++) {
            total_blocks += part_info->blocks_dpu[c * bdbcsrMtx->horz_partitions + i];
            total_nnzs += part_info->nnzs_dpu[c * bdbcsrMtx->horz_partitions + i];
        }
    }
    assert(total_blocks == bdbcsrMtx->nblocks && "Invalid partitioning!");
    assert(total_nnzs == bdbcsrMtx->nnz && "Invalid partitioning!");
}


#if BLNC_TSKLT_BLOCK
/** 
 * @brief load-balance blocks (block-row granularity) across tasklets
 */
void partition_tsklt_by_block(struct BDBCSRMatrix *bdbcsrMtx, struct partition_info_t *part_info, int dpu, int nr_of_tasklets, int nr_of_dpus, int acc_blocks, int prev_block_rows_dpu, int block_rows_dpu, int dpu_col_indx) {
    ...
    for (i = prev_block_rows_dpu; i < prev_block_rows_dpu + block_rows_dpu; i++) {
        ...
        start_block = bdbcsrMtx->browptr[dpu_col_indx * (bdbcsrMtx->num_block_rows + 1) + i];
        end_block = bdbcsrMtx->browptr[dpu_col_indx * (bdbcsrMtx->num_block_rows + 1) + i+1];
        for (j = start_block; j < end_block; j++) {
            curr_nnz += bdbcsrMtx->nnz_per_block[acc_blocks + j];
            curr_block++;
        }
        if (curr_block >= block_per_split) {
            ++split_cnt;
 ...
 */ 
void partition_tsklt_by_nnz(struct BDBCSRMatrix *bdbcsrMtx, struct partition_info_t *part_info, int dpu, int nr_of_tasklets, int nr_of_dpus, int acc_blocks, int prev_block_rows_dpu, int block_rows_dpu, int dpu_col_indx) {
