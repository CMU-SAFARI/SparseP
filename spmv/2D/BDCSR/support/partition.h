...
struct partition_info_t {
    ...
    uint32_t *max_cols_per_rank; // max cols among 64 DPUs per rank
 ...
 */
struct partition_info_t *partition_init(struct BDCSRMatrix *bdcsrMtx, uint32_t nr_of_dpus, uint32_t max_ranks, uint32_t nr_of_tasklets) {
    struct partition_info_t *part_info;
    part_info = (struct partition_info_t *) malloc(sizeof(struct partition_info_t));

    part_info->row_split = (uint32_t *) malloc(bdcsrMtx->vert_partitions * (bdcsrMtx->horz_partitions + 2) * sizeof(uint32_t));
 ...
 */
void partition_by_nnz(struct BDCSRMatrix *bdcsrMtx, struct partition_info_t *part_info) {

    uint32_t c, i;
    for(c = 0; c < bdcsrMtx->vert_partitions; c++) {
        // Compute the matrix splits.
        uint32_t sptr_offset = c * (bdcsrMtx->horz_partitions + 1);
        uint32_t rptr_offset = c * (bdcsrMtx->nrows + 1);
        uint32_t nnz_cnt = bdcsrMtx->nnzs_per_vert_partition[c];
        uint32_t nnz_per_split = nnz_cnt / bdcsrMtx->horz_partitions;
        uint32_t curr_nnz = 0;
        uint32_t row_start = 0;
        uint32_t split_cnt = 0;

        part_info->row_split[sptr_offset] = row_start;
        for (i = 0; i < bdcsrMtx->nrows; i++) {
            curr_nnz += bdcsrMtx->drowptr[rptr_offset + i+1] - bdcsrMtx->drowptr[rptr_offset + i];
            if (curr_nnz >= nnz_per_split) {
                row_start = i + 1;
                ++split_cnt;
                if (split_cnt <= bdcsrMtx->horz_partitions) {
        ...
        if (curr_nnz < nnz_per_split && split_cnt <= bdcsrMtx->horz_partitions) {
            split_cnt++;
            part_info->row_split[sptr_offset + split_cnt] = bdcsrMtx->nrows;
        }

        // If there are any remaining rows merge them in last partition
        if (split_cnt > bdcsrMtx->horz_partitions) {
            part_info->row_split[sptr_offset + bdcsrMtx->horz_partitions] = bdcsrMtx->nrows;
        }

        // If there are remaining threads create empty partitions
        for (i = split_cnt + 1; i <= bdcsrMtx->horz_partitions; i++) {
            part_info->row_split[sptr_offset + i] = bdcsrMtx->nrows;
        }
 ...
 */ 
void partition_tsklt_by_nnz(struct BDCSRMatrix *bdcsrMtx, struct partition_info_t *part_info, int dpu, int rows_per_dpu, int nnz_per_dpu, int ptr_offset, int nr_of_tasklets) {
    ...
    if (nnz_per_dpu == 0) {
        ...
        for (t = 0; t < rows_per_dpu; t++) {
            curr_nnz += bdcsrMtx->drowptr[ptr_offset+t+1] - bdcsrMtx->drowptr[ptr_offset+t];
