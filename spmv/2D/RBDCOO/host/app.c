 ...
 */
static void spmv_host(val_dt* y, struct RBDCOOMatrix *A, val_dt* x) {
...
int main(int argc, char **argv) {
    ...
    // Initialize input data - Padding needed
    ...
    A = coo2rbdcoo(B, horz_partitions, vert_partitions);
    freeCOOMatrix(B);
...
#pragma omp parallel for num_threads(p.nthreads) shared(A, x, z, max_rows_per_dpu, c, r) private(t)
