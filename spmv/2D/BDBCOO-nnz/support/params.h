...
static struct Params input_params(int argc, char **argv) {
    ...
    char *rel_dir = "spmv/2D-clean/RBDBCOO-block";
    char *abs_dir = (char *) malloc(1024);
    abs_dir = getcwd(abs_dir, 1024);
    p.fileName =  strcat(strremove(abs_dir, rel_dir), (char *)"matrices/roadNet-TX.mtx");
    p.row_blsize = 4;
    p.col_blsize = 4;
    p.vert_partitions = 2;
    p.nthreads = 1;
    ...
    int opt;
    while((opt = getopt(argc, argv, "h:f:r:c:v:n:s:")) >= 0) {
        switch(opt) {
            case 'h':
                usage();
                exit(0);
