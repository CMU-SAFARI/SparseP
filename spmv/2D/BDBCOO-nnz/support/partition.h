 ...
 */
void partition_by_nnz(struct BDBCOOMatrix *bdbcooMtx, struct partition_info_t *part_info) {
    ...
    for(c = 0; c < bdbcooMtx->vert_partitions; c++) {
        ...
        uint32_t nnz_cnt = bdbcooMtx->nnzs_per_vert_partition[c];
        uint32_t nnz_per_split = nnz_cnt / bdbcooMtx->horz_partitions;
        ...
        if (curr_nnz < nnz_per_split && split_cnt <= bdbcooMtx->horz_partitions) {
            split_cnt++;
