...
/**
 * @brief BDBCOO matrix format 
 * 2D-partitioned matrix with variable-sized tiles and BCOO on each vertical tile
 */
struct BDBCOOMatrix {
    uint32_t nrows;
    uint32_t ncols;
 ...
 */
uint32_t partitionBDBCSRMatrix(struct BDBCSRMatrix *bdbcsrMtx, uint32_t low, uint32_t high, uint64_t total_blocks) {

    uint32_t pivot = bdbcsrMtx->bcolind[total_blocks + high]; 
    int32_t i = low - 1;
    uint32_t temp_ind;
    val_dt temp_val;

    for(uint32_t j = low; j <= high - 1; j++) {
        if(bdbcsrMtx->bcolind[total_blocks + j] < pivot) {
            i++;
            // swap(i, j)
            temp_ind = bdbcsrMtx->bcolind[total_blocks + i];
            bdbcsrMtx->bcolind[total_blocks + i] = bdbcsrMtx->bcolind[total_blocks + j];
            bdbcsrMtx->bcolind[total_blocks + j] = temp_ind;
            for(uint32_t r=0; r<bdbcsrMtx->row_block_size; r++) {
                for(uint32_t c=0; c<bdbcsrMtx->col_block_size; c++) {
                    temp_val = bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + i * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c];
                    bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + i * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c] = bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + j * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c];
                    bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + j * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c] = temp_val;
                }
            }
        }
    }
    ...
    // swap(i+1, high)
    temp_ind = bdbcsrMtx->bcolind[total_blocks + i+1];
    bdbcsrMtx->bcolind[total_blocks + i+1] = bdbcsrMtx->bcolind[total_blocks + high];
    bdbcsrMtx->bcolind[total_blocks + high] = temp_ind;
    for(uint32_t r=0; r<bdbcsrMtx->row_block_size; r++) {
        for(uint32_t c=0; c<bdbcsrMtx->col_block_size; c++) {
            temp_val = bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + (i+1) * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c];
            bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + (i+1) * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c] = bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + high * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c];
            bdbcsrMtx->bval[total_blocks * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + high * bdbcsrMtx->row_block_size * bdbcsrMtx->col_block_size + r * bdbcsrMtx->col_block_size + c] = temp_val;
        }
    }
    return (i+1); 
}
...
/**
 * @brief quickSort for matrix in BDBCSR format
 */
void quickSortBDBCSRMatrix(struct BDBCSRMatrix *bdbcsrMtx, int32_t low, int32_t high, uint64_t total_blocks) {

    if (low < high) {
        uint32_t mid = partitionBDBCSRMatrix(bdbcsrMtx, low, high, total_blocks);

        quickSortBDBCSRMatrix(bdbcsrMtx, low, mid - 1, total_blocks); 
        quickSortBDBCSRMatrix(bdbcsrMtx, mid + 1, high, total_blocks); 
    }
}

/**
 * @brief Sort wrapper for matrix in BDBCSR format
 */
void sortBDBCSRMatrix(struct BDBCSRMatrix *bdbcsrMtx) {

    uint64_t total_blocks = 0;
    for (uint32_t c = 0; c < bdbcsrMtx->vert_partitions; c++) {
        uint32_t partition = c;
        uint32_t ptr_offset = partition * (bdbcsrMtx->num_block_rows + 1);

        for(uint32_t n=0; n<bdbcsrMtx->num_block_rows; n++) {
            int32_t low = bdbcsrMtx->browptr[ptr_offset + n]; 
            int32_t high = bdbcsrMtx->browptr[ptr_offset + n+1] - 1; 

            quickSortBDBCSRMatrix(bdbcsrMtx, low, high, total_blocks);
        }
        total_blocks += bdbcsrMtx->blocks_per_vert_partition[partition];
    }
}
...
/**
 * @brief convert matrix from BDBCSR to BDBCOO format 
 * @param matrix in BDBCSR format
 * @param horz_partitions, vert_partitions, row_block_size, col_block_size
 */
struct BDBCOOMatrix *bdbcsr2bdbcoo(struct BDBCSRMatrix *bdbcsrMtx) {
    struct BDBCOOMatrix *bdbcooMtx;
    bdbcooMtx = (struct BDBCOOMatrix *) malloc(sizeof(struct BDBCOOMatrix));

    bdbcooMtx->nrows = bdbcsrMtx->nrows;
    bdbcooMtx->ncols = bdbcsrMtx->ncols;
    bdbcooMtx->nnz = bdbcsrMtx->nnz;
    bdbcooMtx->nblocks = bdbcsrMtx->nblocks;
    bdbcooMtx->npartitions = bdbcsrMtx->npartitions; // Equal with column partitions
    bdbcooMtx->horz_partitions = bdbcsrMtx->horz_partitions;
    bdbcooMtx->vert_partitions = bdbcsrMtx->vert_partitions;
    bdbcooMtx->max_tile_width = bdbcsrMtx->max_tile_width;
    bdbcooMtx->num_block_rows = bdbcsrMtx->num_block_rows;
    bdbcooMtx->num_block_cols = bdbcsrMtx->num_block_cols;
    bdbcooMtx->num_rows_left = bdbcsrMtx->num_rows_left;
    bdbcooMtx->row_block_size = bdbcsrMtx->row_block_size;
    bdbcooMtx->col_block_size = bdbcsrMtx->col_block_size;


    bdbcooMtx->nnzs_per_column = (uint32_t *) malloc((bdbcooMtx->ncols) * sizeof(uint32_t));
    bdbcooMtx->nnzs_per_vert_partition = (uint32_t *) malloc(bdbcooMtx->vert_partitions * sizeof(uint32_t));
    bdbcooMtx->blocks_per_vert_partition = (uint32_t *) malloc(bdbcooMtx->vert_partitions * sizeof(uint32_t));
    bdbcooMtx->vert_tile_widths = (uint32_t *) malloc((bdbcooMtx->vert_partitions + 1) * sizeof(uint32_t));
    bdbcooMtx->bind = (struct bind_t *) malloc(bdbcooMtx->nblocks * sizeof(struct bind_t));
    bdbcooMtx->bval = (val_dt *) malloc((bdbcooMtx->nblocks + 1) * bdbcooMtx->row_block_size * bdbcooMtx->col_block_size * sizeof(val_dt));
    bdbcooMtx->nnz_per_block = (uint32_t *) malloc(bdbcooMtx->nblocks * sizeof(uint32_t));

    memcpy(bdbcooMtx->nnzs_per_column, bdbcsrMtx->nnzs_per_column, bdbcsrMtx->ncols * sizeof(uint32_t));
    memcpy(bdbcooMtx->nnzs_per_vert_partition, bdbcsrMtx->nnzs_per_vert_partition, bdbcsrMtx->vert_partitions * sizeof(uint32_t));
    memcpy(bdbcooMtx->blocks_per_vert_partition, bdbcsrMtx->blocks_per_vert_partition, bdbcsrMtx->vert_partitions * sizeof(uint32_t));
    memcpy(bdbcooMtx->vert_tile_widths, bdbcsrMtx->vert_tile_widths, (bdbcsrMtx->vert_partitions + 1) * sizeof(uint32_t));
    memcpy(bdbcooMtx->nnz_per_block, bdbcsrMtx->nnz_per_block, bdbcsrMtx->nblocks * sizeof(uint32_t));
    ...
    for (uint32_t c = 0; c < bdbcsrMtx->vert_partitions; c++) {
        ...
        for(uint64_t n=0; n<bdbcsrMtx->num_block_rows; n++) {
            for(uint64_t i=bdbcsrMtx->browptr[ptr_offset + n]; i<bdbcsrMtx->browptr[ptr_offset + n+1]; i++){
                bdbcooMtx->bind[total_blocks + i].rowind = n;
                bdbcooMtx->bind[total_blocks + i].colind = bdbcsrMtx->bcolind[total_blocks + i];
            }
        }
        total_blocks += bdbcsrMtx->blocks_per_vert_partition[partition];
    }

    memcpy(bdbcooMtx->bval, bdbcsrMtx->bval, (bdbcooMtx->nblocks + 1) * bdbcooMtx->row_block_size * bdbcooMtx->col_block_size * sizeof(val_dt));
...
/**
 * @brief deallocate matrix in BDBCOO format 
 * @param matrix in BDBCOO format
 */  
void freeBDBCOOMatrix(struct BDBCOOMatrix *bdbcooMtx) {
    free(bdbcooMtx->nnzs_per_column);
    free(bdbcooMtx->nnzs_per_vert_partition);
    free(bdbcooMtx->blocks_per_vert_partition);
    free(bdbcooMtx->nnz_per_block);
    free(bdbcooMtx->vert_tile_widths);
    free(bdbcooMtx->bind);
    free(bdbcooMtx->bval);
    free(bdbcooMtx);
}
