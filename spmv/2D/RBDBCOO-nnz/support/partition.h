 ...
 */
void partition_by_nnz(struct RBDBCOOMatrix *rbdbcooMtx, struct partition_info_t *part_info) {
    ...
    for(c = 0; c < rbdbcooMtx->vert_partitions; c++) {
        ...
        uint32_t nnz_cnt = rbdbcooMtx->nnzs_per_vert_partition[c];
        uint32_t nnz_per_split = nnz_cnt / rbdbcooMtx->horz_partitions;
        ...
        if (curr_nnz < nnz_per_split && split_cnt <= rbdbcooMtx->horz_partitions) {
            split_cnt++;
