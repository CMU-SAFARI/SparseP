 ...
 */
void partition_by_nnz(struct BDBCSRMatrix *bdbcsrMtx, struct partition_info_t *part_info) {
    ...
    for(c = 0; c < bdbcsrMtx->vert_partitions; c++) {
        ...
        uint32_t nnz_cnt = bdbcsrMtx->nnzs_per_vert_partition[c];
        uint32_t nnz_per_split = nnz_cnt / bdbcsrMtx->horz_partitions;
        ...
        if (curr_nnz < nnz_per_split && split_cnt <= bdbcsrMtx->horz_partitions) {
            split_cnt++;
