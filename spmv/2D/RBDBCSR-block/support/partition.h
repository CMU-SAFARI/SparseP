 ...
 */
void partition_by_block(struct RBDBCSRMatrix *rbdbcsrMtx, struct partition_info_t *part_info) {
    ...
    for(c = 0; c < rbdbcsrMtx->vert_partitions; c++) {
        ...
        uint32_t block_cnt = rbdbcsrMtx->blocks_per_vert_partition[c];
        uint32_t block_per_split = block_cnt / rbdbcsrMtx->horz_partitions;
        ...
        if (curr_block < block_per_split && split_cnt <= rbdbcsrMtx->horz_partitions) {
            split_cnt++;
