...
/** 
 * @brief load-balance nnz at block granularity across DPUs
 */
void partition_by_nnz(struct BCOOMatrix *bcooMtx, struct partition_info_t *part_info, int nr_of_dpus) {
    ...
    part_info->block_split[0] = block_start;
    for (i = 0; i < bcooMtx->num_blocks; i++) {
        curr_nnz += bcooMtx->nnz_per_block[i];
        if (curr_nnz >= nnz_per_split) {
            ++split_cnt;
            block_start = i + 1;
            if (split_cnt <= nr_of_dpus) {
                part_info->block_split[split_cnt] = block_start;
    ...
    if (curr_nnz < nnz_per_split && split_cnt <= nr_of_dpus) {
        part_info->block_split[++split_cnt] = bcooMtx->num_blocks;
        part_info->nnzs_dpu[split_cnt - 1] = curr_nnz;
    }

    // If there are any remaining blocks merge them in last partition
    if (split_cnt > nr_of_dpus) {
        part_info->block_split[nr_of_dpus] = bcooMtx->num_blocks;
    ...
    for (i = split_cnt + 1; i <= nr_of_dpus; i++) {
        part_info->block_split[i] = bcooMtx->num_blocks;
    ...
    assert(part_info->block_split[nr_of_dpus] == bcooMtx->num_blocks && "Invalid partitioning!");
