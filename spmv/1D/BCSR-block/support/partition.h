...
/** 
 * @brief load-balance blocks across DPUs (block-row granularity)
 */
void partition_by_blocks(struct BCSRMatrix *bcsrMtx, struct partition_info_t * part_info, int nr_of_dpus) {
    ...
    uint32_t block_cnt = bcsrMtx->num_blocks;
    ...
    if (curr_block < block_per_split && split_cnt <= nr_of_dpus) {
