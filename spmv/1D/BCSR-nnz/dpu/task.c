...
int main_kernel_small_blocks(uint32_t tasklet_id) {
    ...
    uint32_t mram_base_addr_bvalues = (uint32_t) (mram_base_addr_bcolind + (max_blocks * sizeof(uint32_t)));
    ...
    mram_base_addr_bvalues += ((prev_row - global_block_offset) * row_block_size * col_block_size * sizeof(val_dt));
    ...
    mram_read((__mram_ptr void const *) (mram_base_addr_bvalues), cache_val, block_size);
    mram_base_addr_bvalues += block_size;
